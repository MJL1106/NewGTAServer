            ...
            if distance <= 15.0 then
                ...
                if distance <= 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        bowling = true
                        TriggerServerEvent('bowling:start')
                    end
                end
            end
        end
...
RegisterNetEvent('bowling:cancel')
AddEventHandler('bowling:cancel', function(full)
    if full then
...
RegisterNetEvent('bowling:chooseTrack')
AddEventHandler('bowling:chooseTrack', function(occupiedTracks)
            ...
            if IsControlJustReleased(0, 215) then
                for i = 1, #Config['Bowling']['Tracks'] do
                    if Config['Bowling']['Tracks'][i] == tracksLeft[currentTrack] then
                        TriggerServerEvent('bowling:choose', i)
                    end
                end
                break
...
RegisterNetEvent('bowling:play')
AddEventHandler('bowling:play', function(chosenTrack)
    bowling = true 
    DoScreenFadeOut(750)
    ...
    DestroyCam(cam, false)
    ClearPedTasks(me)
    TriggerServerEvent('bowling:end', chosenTrack)
    for k, v in pairs(pins) do DeleteEntity(v) end
    bowling = false
end)
