...
QBCore.Functions.CreateUseableItem("blueprint_ammo", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemByName("blueprint_ammo") then
        local craftItem = "pistol_ammo"
        local addedBlueprint = blueprintUsed(source, craftItem)
        if addedBlueprint then
            Player.Functions.RemoveItem("blueprint_ammo", 1)
        end
    end
end)
QBCore.Functions.CreateUseableItem("blueprint_armor", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemByName("blueprint_armor") then
        local craftItem = "armor"
        local addedBlueprint = blueprintUsed(source, craftItem)
        if addedBlueprint then
            Player.Functions.RemoveItem("blueprint_armor", 1)
        end
    end
end)
QBCore.Functions.CreateUseableItem("blueprint_thermite", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemByName("blueprint_thermite") then
        local craftItem = "thermite"
        local addedBlueprint = blueprintUsed(source, craftItem)
        if addedBlueprint then
            Player.Functions.RemoveItem("blueprint_thermite", 1)
        end
    end
end)
QBCore.Functions.CreateUseableItem("blueprint_vintage", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemByName("blueprint_vintage") then
        local craftItem = "weapon_vintagepistol"
        local addedBlueprint = blueprintUsed(source, craftItem)
        if addedBlueprint then
            Player.Functions.RemoveItem("blueprint_vintage", 1)
        end
    end
end)
    ...
    if craftingBenches[benchId] then
        ...
        for index, blueprint in ipairs(craftingBenches[benchId].blueprints) do
            if blueprint == blueprintToRemove then
                ...
                if blueprintToRemove == 'weapon_vintagepistol' then
                    Player.Functions.AddItem('blueprint_vintage',1) 
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['blueprint_vintage'], "add", 1)
                elseif blueprintToRemove == 'armor' then
                    Player.Functions.AddItem('blueprint_armor',1) 
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['blueprint_armor'], "add", 1)
                elseif blueprintToRemove == 'pistol_ammo' then
                    Player.Functions.AddItem('blueprint_ammo',1) 
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['blueprint_ammo'], "add", 1)
                else
                    Player.Functions.AddItem('blueprint_thermite',1) 
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['blueprint_thermite'], "add", 1)
                end
