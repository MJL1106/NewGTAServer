...
local ThermiteSettings = {
    ['fleeca'] = {
        correctBlocks = 16, -- correctBlocks = Number of correct blocks the player needs to click
        ...
        timetoLose = 30 -- timetoLose = maximum time after timetoshow expires for player to select the right blocks
    },
    ['paleto'] = {
        correctBlocks = 16,
        incorrectBlocks = 4,
...
LootTrolly = function(bank, index) -- Globally used
    ...
    if Config.Banks[bank].trolly[index].type == 'money' then
        ...
        -- Reward
        TriggerServerEvent('qb-bankrobbery:server:TrollyReward', Config.Banks[bank].type, bank, index)
    elseif Config.Banks[bank].trolly[index].type == 'gold' then
        ...
        -- Reward
        TriggerServerEvent('qb-bankrobbery:server:TrollyReward', 'gold', bank, index)
    end
    -- evidence
    if math.random(1, 100) <= 65 and not QBCore.Functions.IsWearingGloves() then
...
LootLocker = function(bank, index) -- Globally used
    ...
    if hasItem then
        ...
        QBCore.Functions.Progressbar("open_locker_drill", "Unlocking safe..", 41000, false, true, {
            ...
            -- RECEIVE ITEM
            TriggerServerEvent('qb-bankrobbery:server:LockerReward', Config.Banks[bank].type, bank, index)
