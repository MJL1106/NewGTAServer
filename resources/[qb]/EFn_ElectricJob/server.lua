...
AddEventHandler('GetPaid', function(homie)
    ...
    elseif Config.Framework == 'qbcore' then
        ...
        local eKit = 'electronickit'
        local trojUsb = 'trojan_usb'
        ...
        if Config.Payment == 'cash' then
            ...
            if chance <= 10 then
                Player.Functions.AddItem(eKit, 1, false)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[eKit], 'add', 1)
            elseif chance >=11 and chance <=21 then
                Player.Functions.AddItem(trojUsb, 1, false)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[trojUsb], 'add', 1)
            end
        elseif Config.Payment == 'bank' then
