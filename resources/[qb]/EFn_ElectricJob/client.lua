...
giveJob = function()
    ...
    activeJob:onPointInOut(PolyZone.getPlayerPosition, function(isPointInside, point)
        if isPointInside and duty then
            atJobSite = true
            -- Generate a unique name for the zone to avoid conflicts
            local zoneName = 'PoleUse' .. math.random(1,100)
            activeZoneId = zoneName -- Store the zone's identifier
            ...
            if activeZoneId then
                exports['qb-target']:RemoveZone(activeZoneId) -- Remove the zone when not at the job site
                activeZoneId = nil -- Reset the variable after removing the zone
            end
            atJobSite = false
            HideTextUI()
        end
