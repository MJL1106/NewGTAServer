...
CalculateTime = function(datestr) {
    ...
    return(`${padL(dt.getDate())}.${padL(dt.getMonth()+1)}.${year} - ${padL(dt.getHours())}:${padL(dt.getMinutes())}`
);
...
window.addEventListener('message', function(event) {
    if (event.data.message == "OpenNui") {
        ...
        $(".playerimagetop").css("background-image", "url("+PlayerData.PlayerPhoto+")")
        ...
        $(".playerinfoimg").html(`
            <img src="`+PlayerData.PlayerPhoto+`">
        `)
        ...
        LeaderBoardPage(true, event.data.leaderboard, event.data.lap, event.data.percentage)
    } else if (event.data.message == "closeleaderboard") {
        LeaderBoardPage(false, event.data.leaderboard, event.data.lap, event.data.percentage)
...
SetAnnouncement = function(text, id) {
    $(".annountcement").html(`
        <h1> <strong>Announcement:</strong>`+text+`</h1>
    `)
...
SendNotification = function(text, time) {
    ...
    content = $('<div style="border-radius: 5px; filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25)); border: 1px solid gray;" class="w-[80%] h-fit flex items-center "><div class="w-[7vh] h-[60%] flex justify-center items-center borderright"><img src="./images/exclamation.png"></div><div class="w-[87%] h-fit flex items-center ml-3 mr-3 mb-2 mt-2 annountcement"><h1 style="w-fit; white-space: break-spaces;">'+text+'</h1></div></div>')
            ...
            <div class="w-full h-[18%] hover:bg-[url('./images/history-hover-bg.png')] bg-[url('./images/race-history-bg.png')] bg-no-repeat bg-cover flex mt-2">
                <div class="w-[70%] h-full" >
                    <div class="w-[80%] h-full flex flex-col">
                        <div class="flex w-full h-[30%] history-race-title items-center ml-2" >RACE </div>
                        <div class="flex w-full h-[40%] history-race-name items-center ml-2" >`+RaceHistory[i].racename+`</div>
                        <div class="flex w-full h-[30%] history-race-time items-center ml-2" >FINISHED  <p>&nbsp;| `+time+`</p></div>
                    </div>
                </div>
                ...
                <div class="w-[15%] h-full flex justify-center items-center" style="background: rgba(0, 0, 0, 0.65);">
                    <p class="playerRaceRank">`+rank+`<span>&nbsp;ST</span></p>
                </div>
            </div>
        `)
        } else if (rank == 2) {
            ...
            <div class="w-full h-[18%] hover:bg-[url('./images/history-hover-bg.png')] bg-[url('./images/race-history-bg.png')] bg-no-repeat bg-cover flex mt-2">
                ...
                <div class="w-[15%] h-full flex justify-center items-center" style="background: rgba(0, 0, 0, 0.65);">
                    <p class="playerRaceRank">`+rank+`<span>&nbsp;ND</span></p>
                </div>
            </div>
        `)
        } else if (rank == 3) {
            ...
            <div class="w-full h-[18%] hover:bg-[url('./images/history-hover-bg.png')] bg-[url('./images/race-history-bg.png')] bg-no-repeat bg-cover flex mt-2">
                ...
                <div class="w-[15%] h-full flex justify-center items-center" style="background: rgba(0, 0, 0, 0.65);">
                    <p class="playerRaceRank">`+rank+`<span>&nbsp;RD</span></p>
                </div>
            </div>
        `)
        } else {
            ...
            <div class="w-full h-[18%] hover:bg-[url('./images/history-hover-bg.png')] bg-[url('./images/race-history-bg.png')] bg-no-repeat bg-cover flex mt-2">
                ...
                <div class="w-[15%] h-full flex justify-center items-center" style="background: rgba(0, 0, 0, 0.65);">
                    <p class="playerRaceRank">`+rank+`<span>&nbsp;TH</span></p>
                </div>
            </div>
        `)
        }
...
StartRaceTimeCounter = function() {
    ...
    setInterval(function () {
      ...
      $(".leaderboardtimeest").html(hours + ":" + minutes + ":" + seconds + " s ")
    }, 1000); 
}
            ...
            if (leaderboard[i]) {
                ...
                if (PlayerData.source == leaderboard[i].source) {
                    ...
                    <div class="w-full h-1/5 flex justify-center items-center gap-3" >
                        <div class="w-[20%] h-full flex justify-center items-center text-white" ><p class="leaderboard-rank-user">0`+ranking+`&nbsp;<p style="font-size: 3vh; color: #FD3A69 !important;">·</p></p></div>
                        <div class="w-full h-[80%] flex gap-1 items-center" style="border: 0.3vh solid #FE3968; border-radius: 5px; background: linear-gradient(90.18deg, rgba(0, 0, 0, 0.76) 12.54%, rgba(0, 0, 0, 0.76) 101.68%);">
                            ...
                            <div class="w-[22.5%] h-full  ml-2 overflow-hidden" style="clip-path: polygon(25% 0%, 100% 0%, 75% 100%, 0% 100%);"><img src='`+(leaderboard[i].playerphoto)+`'></div>
                            <p class="text-[1.5vh] w-[50%] h-full flex items-center" >`+leaderboard[i].CharacterName+`</p>
                            <p class="text-[1.3vh] w-[30%] h-full flex items-center justify-center" >`+calc+` mil</p>
                        </div>
                    </div>
                    `)
                } else {
                    if (leaderboard[i].finished) {
                        $(".playerlistleaderboard").append(`
                        <div class="w-full h-1/5 flex justify-center items-center gap-3" >
                            <div class="w-[20%] h-full flex justify-center items-center text-white" ><p class="leaderboard-rank" >0`+ranking+`</p></div>
...
SetIncomingRace = function(incomingrace) {
    $(".incomingracetext").html(`
                <p style="font-size: 1vh; color: #DC355D; font-family: 'gilroy';">Join any race!</p>
            `) 
    ...
    if (PlayerData) {
        if (incomingrace) {
            ...
            `)
            $(".incomingracetext").html(`
                <p style="font-size: 1vh; color: #DC355D; font-family: 'gilroy';">`+incomingrace.data.Name+` | `+GetTimeDifference(incomingrace.data.Others.Time)+` starting!</p>
            `) 
...
UpdateInterval = function(time) {
    ...
    $.post("https://gfx-racing/updateinterval", JSON.stringify({timediff : diff}));
}
                ...
                <div class="w-full h-[4vh] flex" >
                    <div class="w-full h-full flex ranking-index-div nottop5">
                        <div class="w-1/4 h-full ranking-PlayerInfo flex items-center gap-2">
                            <div class="w-1/3 h-[80%] ml-[0.25vh]  bg-no-repeat bg-cover bg-center leadimg"><img style="object-fit: scale-down" src=`+data[i].playerphoto+`></div>
                            <p class="ranking-playerinfo-playername">`+data[i].charname+`</p>
                        </div>
...
SetPlayerRoutes = function(routes) {    
    ...
    $.each(routes, function(i, index){
        ...
        <div class="w-[80%] h-full flex flex-col routes-container" data=`+index.id+`>
            <div class="flex w-full h-[30%] race-title items-center ml-2" >Route -</div>
            <div class="flex w-full h-[40%] race-name items-center ml-2" >`+index.name+`</div>
            <div class="flex w-full h-[30%] race-time items-center ml-2" ><p style="color: #808080; font-size: 1vh;"> <i class="fas fa-hourglass-start"></i> - `+index.StartStreet+`&nbsp;| <i class="fas fa-hourglass"></i> - `+index.FinishStreet+` | `+index.Routes.length+` Area</p></div>
        </div>
        <div class="w-[20%] h-full flex justify-center items-center " onclick=DeleteRoute(`+index.id+`)>
            <i class="fas fa-trash delet"></i>
        </div>
    </div>
        `)
        $(".routesmenu").find("[data="+index.id+"]").data("route", index.Routes)
    })
}
...
AddRace = function(data, route, id) {
    ...
    $(".activelist").prepend(`
    <div class="w-full h-[18%] hover:bg-[url('./images/hover-bg.png')] active-races-cont bg-[url('./images/active-races-bg.png')] mt-2 bg-no-repeat bg-cover flex racelist" id=`+id+` >
                                    <div class="w-[80%] h-full flex flex-col">
                                        <div class="flex w-full h-[30%] race-title items-center ml-2" >Race</div>
                                        <div class="flex w-full h-[40%] race-name items-center ml-2" >`+data.Name+`</div>
                                        <div class="flex w-full h-[30%] race-time items-center ml-2" >ACTIVED | <p>&nbsp;`+date+`</p></div>
                                    </div>
    ...
    $(".activelist").find("[id="+id+"]").data("routes", route)
    $(".activelist").find("[id="+id+"]").data("data", data)
    $(".activelist").find("[id="+id+"]").data("date", date)
    $(".activelist").find("[id="+id+"]").data("raceid", id)
}

ChangeInput = function(domtype, value) {
    if (domtype == "range") {
        $(".playerlength").html( value+ '<h1 class="input-desc">&nbsp;Players</h1>')
        if (Number(value) >= 2) {
        ...
        if (nvalue != "") {
            ...
            <img src="./images/success.png">
                                        ...
                                        <p style="color: white;">GIVE NAME TO RACE</p>
            `)
        ...
        if(value == ""){
            ...
            <div class="w-[8%] h-full flex justify-center items-center">
                                        ...
                                        <p style="color: white;">SET A REWARD</p>
            `)
    ...
    if (CheckList.Others) {
        if (CheckList.Others.Time && CheckList.Others.Players && CheckList.Others.Route) {
            $(".othersettings").html(`
            <div class="w-[8%] h-full flex justify-center items-center">
                                            <img src="./images/success.png">
                                        </div>
                                        <p style="color: white;">OTHER SETTINGS</p>
            `)
        } else {
            $(".othersettings").html(`
            <div class="w-[8%] h-full flex justify-center items-center">
...
$(document).on("dblclick", "#map", function(e) {
    if (page == "races") {return}
    var latLng = mymap.mouseEventToLatLng(e.originalEvent);
    id = RouteList.length+1
    marker = L.marker([latLng.lat,latLng.lng], {icon: customIcon(1), draggable: 'true', id : id}).addTo(mymap).bindPopup("Route");
    RouteList.push(
        {
            id : id,x  : latLng.lng, y : latLng.lat, marker : marker
        }
    )
    TestFunct()
    $.post('https://gfx-racing/AddMarker', JSON.stringify({x : latLng.lng, y : latLng.lat}));
});
    ...
    for (i = 0; i < RouteList.length; i++) {
        ...
        if (marker && !RouteList[i].eventcreated) {
            ...
            marker.on('dragend', function(event) {
                    ...
                    $.post('https://gfx-racing/UpdateMarker', JSON.stringify({x : position.lng, y : position.lat, id : marker.options.id} ));
            });
        }
    }
}
...
$(document).on("click",".routes-container",function() {
    ...
    $.each(route, function(i, index){
        L.marker([index.y,index.x], {icon: customIcon(1)}).addTo(mymap).bindPopup("Route");
    })
});
...
$(document).on("click",".showfirstpick",function() {
    ...
    if (toggleraces[id]) {
        L.marker([y,x], {
                ...
                className : "gfx",
                html: `
                    <div class='showfirstpick flex items-center' id=`+id+`>
                ...
                `
                    ...
                    $(".gfx").find("[id="+id+"]").data("id", id)
                    $(".gfx").find("[id="+id+"]").data("x", x)
                    $(".gfx").find("[id="+id+"]").data("y", y)
                    $(".gfx").find("[id="+id+"]").data("price", price)
                    $(".gfx").find("[id="+id+"]").data("name", name)
        ...
        L.marker([y,x], {
            ...
            className : "gfx",
            html: `
            <div class='vectortextside-active showfirstpick' id=`+id+`>
            
            
             <div class="race-reward-contain">
                 <img src="images/cupreward.png" alt=""><p>`+price+`$</p>
             </div>
            ...
            <button class="racejoin-way-join" raceid=`+id+`><p>JOIN</p></button>
            </div>
...
ShowRaceStarts = function() {
    $.each(ActiveRaces, function(i, index){
            L.marker([index.route[0].y,index.route[0].x], {
                ...
                className : "gfx",
                html: `
                    <div class='showfirstpick flex items-center' id=`+index.id+`>
                        <div class="vectorfiled">
                            <img src='./images/vector.png'>
                        </div>
                        <div class="vectortextside">
        ...
        $(".gfx").find("[id="+index.id+"]").data("id", index.id)
        $(".gfx").find("[id="+index.id+"]").data("x", index.route[0].x)
        $(".gfx").find("[id="+index.id+"]").data("y", index.route[0].y)
        $(".gfx").find("[id="+index.id+"]").data("price", index.data.Reward)
        $(".gfx").find("[id="+index.id+"]").data("name", index.data.Name)
    })
...
CreateRace = function() {
    if (CheckList.Others.Time && CheckList.Others.Players && CheckList.Others.Route && CheckList.Name && CheckList.Reward) {
        $.post('https://gfx-racing/CreateRace', JSON.stringify({data : CheckList, luadate : new Date(CheckList.Others.Time).getTime() }));
        $('#range').val(0);
        ...
        if (!CheckList.Others.Time) {
            SendNotification('You need to set date!', 2500)
        } else if (!CheckList.Others.Players) {
            SendNotification('You need to set minimum players!', 2500)
        } else if (!CheckList.Others.Route) {
            SendNotification('You need to set route!', 2500)
        } else if (!CheckList.Name) {
            SendNotification('You need to set race name!', 2500)
        } else if (!CheckList.Reward) {
            SendNotification('You need to set race reward!', 2500)
        }
    }
}
        ...
        $("#map").append(`
                <div id="desc-side-map">
                    <div class="w-full h-full flex gap-1 justify-end">
                        <div class="w-full h-full flex justify-center items-center selectrouteindex" id=`+id+` style="border: 1px solid rgba(255, 255, 255, 0.16); background:rgba(0, 0, 0, 0.44); border-radius: 0.3vw; color:rgba(254, 57, 103, 0.692); font-size:2.2vh;">
                            SELECT ROUTE
                        </div>
                    </div>
                </div>
