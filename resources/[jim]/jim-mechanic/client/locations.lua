			...
			if Config.RequiresJob then
				if v.stash then
					for l, b in pairs(v.stash) do
						...
						{ options = { 
							{ 
								event = "jim-mechanic:client:Safe", 
								icon = "fas fa-cogs", 
								label = Loc[Config.Lan]["repair"].browse, 
								parameters = { job = v.job } -- Ensure 'parameters' includes the job
							}, 
						},
						distance = 2.0 })
					end
...
RegisterNetEvent('jim-mechanic:client:Safe', function(data) 
    local job = data.parameters and data.parameters.job
    if job then
        print(job) -- This should now print the job information
        TriggerEvent("inventory:client:SetCurrentStash", job.."Safe") 
        TriggerServerEvent("inventory:server:OpenInventory", "stash", job.."Safe", { maxweight = 4000000, slots = 50, }) 
    else
