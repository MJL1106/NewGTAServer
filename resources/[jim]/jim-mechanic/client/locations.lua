...
RegisterNetEvent('jim-mechanic:client:Safe', function(data)
    -- Attempt to use the job field from data
    local job = data.job

    -- Fallback to player's current job if job is nil
    if not job or job == "nil" then
        local PlayerData = QBCore and QBCore.Functions.GetPlayerData()
    ...
    if job == "unknown" then return end

    -- Trigger inventory events with the resolved job
    TriggerEvent("inventory:client:SetCurrentStash", job .. "Safe")
    TriggerServerEvent("inventory:server:OpenInventory", "stash", job .. "Safe", {
        maxweight = 4000000,
        slots = 50,
    })
end)
